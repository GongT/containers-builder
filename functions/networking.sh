...
function network_use_interface() {
	...
	if [[ ${NET_NAMESPACE+found} != found ]]; then
		local NET_NAMESPACE
		NET_NAMESPACE="service-$(_unit_get_name)"
	fi

	_net_set_type "exclusive"
	_unit_podman_network_arg "--network=ns:/var/run/netns/$NET_NAMESPACE" --no-hosts

	SCRIPT=$(install_script "$COMMON_LIB_ROOT/container-tools/network-move-interface.sh")
...
function network_use_veth() {
	# use "podman" network
	local BRIDEG_NAME=$1 PORT_FORWARD=("$@")
	local SCRIPT
	...
	unit_unit After "firewalld.service" "$BRIDEG_NAME"
...
function network_use_container() {
	...
	SCRIPT=$(install_script "$COMMON_LIB_ROOT/container-tools/network-wait-continaer.sh")
	unit_hook_start "+/usr/bin/bash $SCRIPT '$CONTAINER_NAME'"
...
function network_use_pod() {
	...
	if [[ -e "$PODMAN_QUADLET_DIR/$PODNAME.pod" ]]; then
		unit_depend "service-pod-$PODNAME.service"
	else
		info_warn "pod may not exists: $PODNAME"

		local SCRIPT
		SCRIPT=$(install_script "$COMMON_LIB_ROOT/container-tools/network-wait-pod.sh")
		unit_hook_start "+/usr/bin/bash $SCRIPT '$PODNAME'"
	fi
